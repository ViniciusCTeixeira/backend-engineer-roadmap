...
13. **Local-only repository validation for V1** — The roadmap keeps `scripts/validate_repo.py` and its test suite as mandatory local quality gates, but does not run hosted GitHub Actions for this repository. This avoids hosted CI billing while preserving deterministic validation. GitHub Actions remains part of the learning curriculum and may be practiced in dedicated sandbox/project exercises rather than as always-on CI for this roadmap repository.
