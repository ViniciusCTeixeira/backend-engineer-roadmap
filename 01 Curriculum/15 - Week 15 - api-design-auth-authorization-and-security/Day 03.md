---
type: daily-plan
schema_version: 1
id: task-w15-d03-api-design-auth-authorization-and-security
week: 15
day: 3
date: null
track: core
skill_ids:
- api-design
- authentication
- authorization
- oauth-oidc
mode: SOLO
estimated_minutes: 105
technology_depth: core
resource_ids:
- rfc6749-oauth2
- rfc9700-oauth-security
- openid-connect-core
deliverables:
- OAuth/OIDC flow diagram
- build-vs-buy note
- Project B identity requirements
review_policy: adaptive
prerequisites:
- week-14
status: planned
---

# Day 03 — Understand OAuth 2.0 and OpenID Connect Roles Without Reinventing Them

## Outcome

> Map OAuth/OIDC roles and authorization-code+PKCE flow; define mature-library/provider boundary.

## Timebox

| Block | Mode | Minutes |
|---|---|---:|
| Primary-source study | `SOLO` | 20 |
| Hands-on reasoning / lab | `SOLO` | 40 |
| Project / evidence update | `SOLO` | 15 |
| Technical English / explanation | `SOLO` | 10 |
| Daily micro-assessment | `SOLO` | 20 |
| **Total** |  | **105** |

Assessment time is included.

## Resources

- `rfc6749-oauth2`
- `rfc9700-oauth-security`
- `openid-connect-core`

Resource IDs resolve through `09 Resources/catalog.yaml`.

## Activities

### Primary-source study — 20 min — `SOLO`

Study the relevant primary source specifically for: Map OAuth/OIDC roles and authorization-code+PKCE flow; define mature-library/provider boundary.

### Hands-on reasoning / lab — 40 min — `SOLO`

Map OAuth/OIDC roles and authorization-code+PKCE flow; define mature-library/provider boundary.

### Project / evidence update — 15 min — `SOLO`

Connect the result to Project A/B or preserve a reproducible standalone artifact.

### Technical English / explanation — 10 min — `SOLO`

Explain one important decision, assumption, or failure mode in English.

### Daily micro-assessment — 20 min — `SOLO`

Complete the scored transfer prompt without assistance.

## Deliverables

- OAuth/OIDC flow diagram
- build-vs-buy note
- Project B identity requirements

## Daily assessment

Run `02 Daily Assessments/Week 15/Day 03.md`. Store learner attempts privately.

## Review hook

Create D+1/D+7 reviews from demonstrated errors; add D+30 transfer review when justified. Gate remediation is private and preserves the raw attempt.

## AI integrity

Scored attempts are SOLO. HYBRID activities freeze the SOLO baseline before agent use and require independent validation afterward.

## Completion rule

Reading or agent conversation alone is not completion; required evidence must exist.
