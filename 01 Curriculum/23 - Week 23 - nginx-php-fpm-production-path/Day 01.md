...
type: daily-plan
schema_version: 1
id: task-w23-d01-nginx-php-fpm-production-path
week: 23
day: 1
date: null
track: core
...
- docker-get-started
deliverables:
- Build the Nginx→FPM request path evidence
...
# Day 01 — Build the Nginx→FPM request path
...
Read the relevant primary source with the specific question: Build the Nginx→FPM request path.

### Hands-on reasoning / lab — 40 min — `SOLO`

Build the Nginx→FPM request path. Start with a prediction/design before execution or lookup.
...
Run `02 Daily Assessments/Week 23/Day 01.md`. Store learner responses only in private state.
